...
CLI_HELP_MSG = f"""
    ...
    1. Train a ultralytics model for 10 epochs with an initial learning_rate of 0.01
        yolo train data=coco8.yaml model=yolo11n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at images size 320:
        yolo predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Val a pretrained ultralytics model at batch-size 1 and images size 640:
        yolo val model=yolo11n.pt data=coco8.yaml batch=1 imgsz=640

    4. Export a YOLO11n classification model to ONNX format at images size 224 by 128 (no TASK required)
...
def handle_streamlit_inference():
    """
    Open the Ultralytics Live Inference Streamlit app for real-time object ultralytics.

    This function initializes and runs a Streamlit application designed for performing live object ultralytics using
...
def entrypoint(debug=""):
    ...
    Examples:
        Train a ultralytics model for 10 epochs with an initial learning_rate of 0.01:
        >>> entrypoint("train data=coco8.yaml model=yolo11n.pt epochs=10 lr0=0.01")

        Predict a YouTube video using a pretrained segmentation model at images size 320:
        >>> entrypoint("predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320")

        Validate a pretrained ultralytics model at batch-size 1 and images size 640:
