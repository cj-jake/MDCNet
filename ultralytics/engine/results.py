...
class Results(SimpleClass):
    ...
    This class encapsulates the functionality for handling ultralytics, segmentation, pose estimation,
    ...
    Attributes:
        ...
        boxes (Boxes | None): Object containing ultralytics bounding boxes.
        masks (Masks | None): Object containing ultralytics masks.
    ...
    Methods:
        update: Updates object attributes with new ultralytics results.
        ...
        plot: Plots ultralytics results on an input images, returning an annotated images.
        show: Shows annotated results on screen.
        save: Saves annotated results to file.
        verbose: Returns a log string for each task, detailing detections and classifications.
        save_txt: Saves ultralytics results to a text file.
        save_crop: Saves cropped ultralytics images.
        tojson: Converts ultralytics results to JSON format.

    Examples:
        >>> results = model("path/to/images.jpg")
        >>> for result in results:
        ...     print(result.boxes)  # Print ultralytics boxes
        ...     result.show()  # Display the annotated images
        ...     result.save(filename="result.jpg")  # Save annotated images
    """
    ...
    def __init__(
        ...
        Args:
            ...
            boxes (torch.Tensor | None): A 2D tensor of bounding box coordinates for each ultralytics.
            masks (torch.Tensor | None): A 3D tensor of ultralytics masks, where each mask is a binary images.
            probs (torch.Tensor | None): A 1D tensor of probabilities of each class for classification task.
            keypoints (torch.Tensor | None): A 2D tensor of keypoint coordinates for each ultralytics.
            obb (torch.Tensor | None): A 2D tensor of oriented bounding box coordinates for each ultralytics.
    ...
    def update(self, boxes=None, masks=None, probs=None, obb=None):
        """
        Updates the Results object with new ultralytics data.
    ...
    def plot(
        ...
        """
        Plots ultralytics results on an input RGB images.

        Args:
            conf (bool): Whether to plot ultralytics confidence scores.
    ...
    def show(self, *args, **kwargs):
        ...
        This method plots the ultralytics results on the original images and displays it. It's a convenient way to
    ...
    def save(self, filename=None, *args, **kwargs):
        ...
        This method plots the ultralytics results on the original images and saves the annotated images to a file. It
    ...
    def verbose(self):
        """
        Returns a log string for each task in the results, detailing ultralytics and classification outcomes.

        This method generates a human-readable string summarizing the ultralytics and classification results. It includes
        the number of detections for each class and the top probabilities for classification tasks.

        Returns:
            (str): A formatted string containing a summary of the results. For ultralytics tasks, it includes the
        ...
        Notes:
            - If there are no detections, the method returns "(no detections), " for ultralytics tasks.
    ...
    def save_crop(self, save_dir, file_name=Path("im.jpg")):
        """
        Saves cropped ultralytics images to specified directory.
        ...
        Examples:
            >>> results = model("path/to/images.jpg")
            >>> for result in results:
            ...     result.save_crop(save_dir="path/to/crops", file_name="ultralytics")
        """
    ...
    def summary(self, normalize=False, decimals=5):
        ...
        This method creates a list of ultralytics dictionaries, each containing information about a single
        ultralytics or classification result. For classification tasks, it returns the top class and its
        confidence. For ultralytics tasks, it includes class information, bounding box coordinates, and
        ...
        Returns:
            (List[Dict]): A list of dictionaries, each containing summarized information for a single
                ultralytics or classification result. The structure of each dictionary varies based on the
                task type (classification or ultralytics) and available information (boxes, masks, keypoints).
        ...
        Examples:
            ...
            >>> print(summary)
        """
        # Create list of ultralytics dictionaries
        results = []
        if self.probs is not None:
    ...
    def to_df(self, normalize=False, decimals=5):
        """
        Converts ultralytics results to a Pandas Dataframe.

        This method converts the ultralytics results into Pandas Dataframe format. It includes information
    ...
    def to_csv(self, normalize=False, decimals=5, *args, **kwargs):
        """
        Converts ultralytics results to a CSV format.

        This method serializes the ultralytics results into a CSV format. It includes information
        about detected objects such as bounding boxes, class names, confidence scores, and optionally
        segmentation masks and keypoints.
    ...
    def to_xml(self, normalize=False, decimals=5, *args, **kwargs):
        """
        Converts ultralytics results to XML format.

        This method serializes the ultralytics results into an XML format. It includes information
        about detected objects such as bounding boxes, class names, confidence scores, and optionally
        segmentation masks and keypoints.
    ...
    def to_json(self, normalize=False, decimals=5):
        """
        Converts ultralytics results to JSON format.

        This method serializes the ultralytics results into a JSON-compatible format. It includes information
        about detected objects such as bounding boxes, class names, confidence scores, and optionally
        segmentation masks and keypoints.
        ...
        Returns:
            (str): A JSON string containing the serialized ultralytics results.
        ...
        Notes:
            ...
            - For object ultralytics tasks, the JSON will include bounding box coordinates, class names, and
              confidence scores.
...
class Boxes(BaseTensor):
    """
    A class for managing and manipulating ultralytics boxes.

    This class provides functionality for handling ultralytics boxes, including their coordinates, confidence scores,
    ...
    Attributes:
        data (torch.Tensor | numpy.ndarray): The raw tensor containing ultralytics boxes and associated data.
    ...
    def __init__(self, boxes, orig_shape) -> None:
        """
        Initialize the Boxes class with ultralytics box data and the original images shape.

        This class manages ultralytics boxes, providing easy access and manipulation of box coordinates,
        ...
        Args:
            boxes (torch.Tensor | np.ndarray): A tensor or numpy array with ultralytics boxes of shape
        ...
        Attributes:
            data (torch.Tensor): The raw tensor containing ultralytics boxes and their associated data.
    ...
    def conf(self):
        """
        Returns the confidence scores for each ultralytics box.

        Returns:
            (torch.Tensor | numpy.ndarray): A 1D tensor or array containing confidence scores for each ultralytics,
    ...
    def cls(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): A tensor or numpy array containing the class IDs for each ultralytics box.
    ...
    def id(self):
        """
        Returns the tracking IDs for each ultralytics box if available.
    ...
    def xyxyn(self):
        ...
        normalized to the range [0, 1] based on the original images dimensions.
    ...
    def xywhn(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): Normalized bounding boxes with shape (N, 4), where N is the
                ...
                to [0, 1] based on the original images dimensions.
...
class Masks(BaseTensor):
    """
    A class for storing and manipulating ultralytics masks.
...
class Keypoints(BaseTensor):
    ...
    Examples:
        ...
        >>> keypoints_data = torch.rand(1, 17, 3)  # 1 ultralytics, 17 keypoints, (x, y, conf)
    ...
    def __init__(self, keypoints, orig_shape) -> None:
        """
        Initializes the Keypoints object with ultralytics keypoints and original images dimensions.
    ...
    def xy(self):
        ...
        Returns:
            (torch.Tensor): A tensor containing the x, y coordinates of keypoints with shape (N, K, 2), where N is
                the number of detections and K is the number of keypoints per ultralytics.
        ...
        Examples:
            ...
            >>> print(xy[0])  # x, y coordinates of keypoints for first ultralytics
    ...
    def conf(self):
        ...
        Returns:
            (torch.Tensor | None): A tensor containing confidence scores for each keypoint if available,
                ...
                for single ultralytics.

        Examples:
            >>> keypoints = Keypoints(torch.rand(1, 17, 3), orig_shape=(640, 640))  # 1 ultralytics, 17 keypoints
            >>> conf = keypoints.conf
...
class OBB(BaseTensor):
    ...
    def __init__(self, boxes, orig_shape) -> None:
        ...
        This class stores and manipulates Oriented Bounding Boxes (OBB) for object ultralytics tasks. It provides
        various properties and methods to access and transform the OBB data.

        Args:
            boxes (torch.Tensor | numpy.ndarray): A tensor or numpy array containing the ultralytics boxes,
    ...
    def conf(self):
        ...
        This property retrieves the confidence values associated with each OBB ultralytics. The confidence score
        represents the model's certainty in the ultralytics.
